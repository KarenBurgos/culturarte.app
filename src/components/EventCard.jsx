...
function EventCard(params) {
  ...
  const [ableToBuy, setAbleToBuy] = useState(null);
  ...
  useEffect(() => {
    localStorage.setItem("eventCard", JSON.stringify(card));
    if(eventDate <= today)
      setAbleToBuy(false);
  }, [card]);

  return (
    <Link onClick={() => {localStorage.setItem("EventId", params.id)}} to="/user/buytickets">
